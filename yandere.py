...
def get_Oldpngfilename(file_path):
	return re.compile('re%20(\d+)%.+?.png').findall(file_path)

def get_Oldjpgfilename(file_path):
	return re.compile('re%20(\d+)%.+?.jpg').findall(file_path)
